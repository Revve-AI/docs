...
The **Analytics** section in the left sidebar contains four built-in dashboards. Each answers a different question:
...
The same section also contains **Dashboards**, where you can create call reports with configurable charts, metrics, filters, and layouts. See [Custom Dashboards](/history-analytics/custom-dashboards).

The Analytics sidebar requires **Manage Analytics**. Current default admin and member roles include it; your workspace may use different grants. See [Roles and Permissions](/governance-access/roles-and-permissions) if the section is missing from your sidebar.

## Overview

The Overview page (titled **Agent Productivity**) gives you a snapshot of your operations. It refreshes automatically every five minutes. You can also click **Refresh** for the latest data. AI Agents and Human Agents use the same fixed refresh interval.
...
The **AI Agents** page (titled **AI Agent Performance**) monitors AI effectiveness, response performance, and escalation patterns. Use the **date range selector** to choose a period (defaults to the last 7 days). Data refreshes automatically every five minutes. A manual **Refresh** control is also available; there is no export.
...
The **Human Agents** page (titled **Human Agent Performance**) tracks team productivity, workload distribution, and response times. Use the **date range selector** to choose a period (defaults to the last 7 days). Data refreshes automatically every five minutes. A manual **Refresh** control is also available; there is no export.

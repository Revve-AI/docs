...
- **Conversation distribution** — a pie chart of conversation volume by channel (Website, SMS, WhatsApp, Messenger, Email, Zalo, Zalo OA) and a stacked bar chart of daily volume by channel.
...
| **AI Fail Rate** | AI evaluation fail rate. |
| **Quality Failed** | Count of conversations that failed evaluation. |
| **Avg Response Time (s)** | Average AI response time in seconds. |

Sort by **AI Fail Rate** to find the agent that most needs prompt or knowledge base work; sort by **Volume** to see where a fix pays off fastest.
...
- **Conversation heatmap** — conversation volume by day of week and hour. Use it to see when visitors are most active.
- **Escalation heatmap** — when handoffs to humans happen, by time of day.

If containment starts falling, check the **escalation heatmap** first: escalations concentrated in specific hours usually point to a traffic pattern (a topic your agent can't handle that spikes at certain times, or after-hours questions it defers) rather than a general quality regression. Escalations spread evenly across the week point back to the agent itself — review recent prompt or knowledge base changes. For what happens after a handoff, see [Human Takeover](/inbox/human-takeover).
...
| **Resolution Rate** | Share of handled conversations that reach resolution. |
...
A sortable per-agent table: **Agent**, **Active Conversations**, **Total Conversations**, **Tickets Assigned**, **Tickets Resolved**, **Avg First Response Time**, **Avg Handle Time**, **Avg Resolution Time**, **Resolution Rate**, **Avg CSAT**.
...
- **Workload distribution** — how conversations and tickets spread across team members.

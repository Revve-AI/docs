...
| **Transfer Call** | A telephony hand-off node (transfer to a phone number, SIP address, or in-app agent). Its settings — destination numbers, bridge/cold transfer, a spoken transfer message — target voice calls; for handing a chat conversation to a human, use the **Human Escalation** toggle instead ([Flow Node Features](/conversational-flow/flow-node-features)). |
...
- [Global Nodes and Prompts](/conversational-flow/global-nodes-and-prompts) -- Learn about nodes that can be reached from any point in the conversation.
- [Flow Node Features](/conversational-flow/flow-node-features) -- Explore per-node capabilities like contact collection and meeting scheduling.

...
- `contact` is display-level data for the dashboard contact record — identification, not authentication. See [Visitor identity](#visitor-identity) below.
...
The widget SDK supports two identity tiers:
...
2. **Identified** — `boot` with `userId`/`email`/`contact`. Convenience-level identification: the dashboard shows a real name and contact record, and `getVisitorId()` returns your `userId`. This is display-level identification, not cryptographic verification — nothing stops a visitor from claiming someone else's ID from the browser console, so don't gate sensitive data on it.

There is no signed-token or server-verified identity option in this SDK — `boot` does not accept anything like a `token` field. If you need cryptographically verified identity, that only exists for the separate Bridge integration (embedding the widget inside a native mobile app over postMessage, with optional RS256 JWT verification) — see [Bridge Configuration](/general-configuration/chat-agent-advanced-settings#bridge-configuration). The Bridge is a different mechanism from the `window.Revve` SDK documented on this page, not an extension of it.
...
- [Chat Agent Advanced Settings](/general-configuration/chat-agent-advanced-settings) — the separate Bridge integration for embedding the widget in a native mobile app, including its own RS256 JWT verification.

...
Rename it right away: click the pencil next to the name in the top bar, type a name that says what the rule does (Enter saves, Esc cancels).
...
| **In (comma-separated)** / **Not In (comma-separated)** | matches / doesn't match any item in a comma-separated list |
...
<Note>
Some rules also show read-only **managed** action cards — [Salesforce](/integrations/connecting-salesforce) or [HubSpot](/integrations/connecting-hubspot-crm) sync steps provisioned by CRM apps. You can't add or edit those in the builder.
</Note>

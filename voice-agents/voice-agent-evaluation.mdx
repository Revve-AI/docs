...
<Note>
Chat Agents have their own "Evaluations" tab, but it's a different feature entirely — pre-ship regression testing against defined test cases (see [Creating Your First Chat Agent](/getting-started/creating-your-first-chat-agent)). This page covers the automatic post-call grading policy, which exists only for Voice Agents.
</Note>
...
3. **Add metrics from your team's library, or create one inline.** Metrics belong to your team, not to a single agent — you can reuse the same metric across multiple Voice Agents. Adding a metric to a group lets you set its **weight** (default 1) and, optionally, a **threshold score** for that policy.

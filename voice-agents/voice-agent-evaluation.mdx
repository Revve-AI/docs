...
This is separate from the **General** / **Model Settings** / **Advanced Settings** tabs covered in [Voice Agent Settings](/voice-agents/voice-agent-settings). It's its own tab because it isn't a call-behavior setting — it's a policy that runs *after* the call, against the finished transcript and tool logs.
...
<Note>
Chat Agents have their own "Evaluations" tab, but it's a different, older feature — pre-ship regression testing against defined test cases (see [Creating Your First Chat Agent](/getting-started/creating-your-first-chat-agent)). This page covers the automatic post-call grading policy, which exists only for Voice Agents.

For pre-ship regression testing that works across both Text and Voice agents, see the [Evaluations](/evaluations/evaluations-overview) product in the left nav — Evaluators, Profiles, Metrics, Scorecards, and Runs, starting with [Creating an Evaluator](/evaluations/creating-an-evaluator).
</Note>
...
- **Scoring** — **Binary** for a straight pass/fail, or **Scale 1-5** when you want gradations. For Scale 1-5, you write a **Score Rubric**: a description of what earns each of score_1 through score_5, specific to that metric. There's no generic 1-bad/5-good scale — you have to tell the judge what a "3" means for *this* metric, or the score is meaningless.

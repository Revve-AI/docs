...
Test success, rejected authorization, missing data, timeout, duplicate submission, and partial failure. [Voice evaluations](/voice-agents/voice-agent-evaluation) can check that the expected tool was called and that the agent responded appropriately. Publish the tested agent version using [Voice Agent Versioning](/voice-agents/voice-agent-versioning); users without **Publish Agents** submit the draft through the [approval workflow](/governance-access/publish-approval-workflow).

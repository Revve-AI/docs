...
[Voice Agent Settings](/voice-agents/voice-agent-settings) covers the conversation-shaping tabs of Advanced Settings — Call Behavior, Conversation, Voicemail, and Call Transfer. This page covers the rest: the capabilities you enable once the agent talks well and you need it to *operate* well — verify identities, retry failed dials, feed your systems, and alert your team.
...
| Setting | Default | When to change |
|---------|---------|----------------|
| **Validation Field Name** | `phoneNumber` | The contact field containing the number to verify against. Required when verification is on — leaving it blank blocks saving. Change it only if your phone number lives in a different field. |
...
Two independent toggles, both **Vietnamese-specific**, that turn messy spoken input into structured data. Each adds one tool to the agent; at runtime both normalize the spoken input and have the agent read the result back digit by digit for confirmation ("có đúng không ạ?"), so transcription mistakes get caught on the call rather than in your database.

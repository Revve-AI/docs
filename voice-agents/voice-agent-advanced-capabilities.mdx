...
[Voice Agent Settings](/voice-agents/voice-agent-settings) covers the conversation-shaping tabs of Advanced Settings — Call Behavior, Conversation, Voicemail, and Call Transfer. This page covers the remaining operational tabs: the capabilities you enable once the agent talks well and you need it to *operate* well — verify identities, retry failed dials, feed your systems, and alert your team.
...
Two independent toggles, both **Vietnamese-specific**, that turn messy spoken input into structured data. Each adds one tool to the agent. License-plate normalization has the agent read the normalized plate back digit by digit for confirmation ("có đúng không ạ?"), so transcription mistakes get caught on the call; address normalization returns the cleaned, structured address for the agent to use in the conversation.

| Setting | Default | When to change |
|---------|---------|----------------|
| **Address Normalization** | Off | Adds the `norm_address` tool, which normalizes and validates a user-provided address. Turn on whenever the agent collects delivery or billing addresses from Vietnamese callers. |
| **License Plate Normalization** | Off | Adds the `norm_license_plate` tool. Turn on for insurance, parking, or fleet use cases where the agent records vehicle plates. |

Each card has an optional **Tool Call Message** — a sentence spoken while the tool runs (e.g. `Dạ vâng, để em ghi nhận địa chỉ ạ.` / `Dạ vâng, để em ghi nhận biển số ạ.`). As with Phone Verification, set it to avoid mid-call silence.
...
There's no per-event selection or signing-secret field — one URL receives the agent's events. If you only need to trigger actions *inside* Revve (tags, assignments, follow-ups), use [automation rules](/automations/automation-rules-overview) instead of standing up an endpoint.

...
| **Tools** | Attach custom tools (API calls, SMS sending) the agent can use mid-call, plus two calendar presets — **Check Calendar Availability** and **Book on the Calendar** — that connect to a Calendly, Cal.com, or Chili Piper calendar so the agent can book an appointment before the call ends. See [Build a Voice Agent That Books Appointments Mid-Call](/guides/appointment-booking-voice-agent) for the full setup. |

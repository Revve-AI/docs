...
<Note>
Voice agents created for Vietnamese-language teams start with different turn-taking defaults, tuned for the language's speech patterns:

| Setting | Vietnamese default | Generic default |
|---------|---------------------|------------------|
| **Turn detection model** | Smart Turn Detection | Voice Activity Detection (VAD) |
| **Smart turn threshold** | 0.45 | 0.7 |
| **Response timing** | Fixed, max 1.8 s / min 0.3 s | Fixed, max 3.0 s / min 0.5 s |
| **Min silence duration** | 0.25 s | 0.3 s |

The lower Smart Turn threshold compensates for Vietnamese being Smart Turn's weakest-performing language — it scores complete Vietnamese utterances lower on average than other languages.

These values are set once, at agent creation, based on the team's language setting — they're not a live runtime fallback. An agent created before this change keeps its original values (threshold 0.65, max delay 4 s, min delay 0.4 s) until you edit and re-save its settings.
</Note>
...
The **Verbatim** default is a gender-conditional template using the `predicted_gender` variable:

```
{{#if predicted_gender = 'male'}}
Anh có thể nói to hơn một xíu được không ạ ?
{{/if}}
{{#if predicted_gender = 'female'}}
Chị có thể nói to hơn một xíu được không ạ ?
{{/if}}
{{#if !predicted_gender}}
Bạn có thể nói to hơn một xíu được không ạ ?
{{/if}}
```

The **Prompt** mode's default instruction: `Người gọi đang nói quá nhỏ. Hãy lịch sự đề nghị họ nói to hơn một chút, xưng hô phù hợp với {{predicted_gender}} (anh/chị), giọng thân thiện và ngắn gọn.`

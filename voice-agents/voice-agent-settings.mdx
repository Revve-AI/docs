...
Native detection that classifies who answered — a human, voicemail, or an IVR phone menu — before the agent starts speaking. Off by default; existing agents keep the legacy LLM-based voicemail detection until you switch. When enabled, the agent can leave a voicemail message instead of hanging up and can navigate IVR menus to reach a person. For outbound campaigns it's usually worth turning on — a personalized message measurably improves callback rates versus silent hangups.

See [Answer Machine Detection](/voice-agents/answer-machine-detection) for setup, the voicemail message, and IVR navigation.
...
4. Only then polish: answer-machine voicemail message, transfer message, pronunciation dictionary.

## What's Next

- [Answer Machine Detection](/voice-agents/answer-machine-detection) — detect voicemail and IVR menus before the agent speaks.

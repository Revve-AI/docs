...
|--------|--------------|----------------|
| **Revve User (in-app agent)** | Routes the call to an available human agent in the Revve app — no phone system needed. The caller hears hold music while waiting for a teammate to accept, the same treatment Bridge Transfer uses for SIP/phone handoffs. | Your team works in Revve anyway. This is the simplest setup: teammates toggle **Accept Calls** on and answer in the browser (see Step 2). |
...
When the agent initiates an in-app transfer, available teammates see an **Incoming Call** card appear anywhere in the app: a pulsing green dot, a bare countdown like "12s" (no longer phrased as "Expires in `{n}`s"), and a drain bar underneath that turns amber in the final 10 seconds. The caller's name or number is the card's title — with an avatar if you have one for them, or a phone icon for an unknown caller — followed by a subtitle reading "via `{callBotName}` · `{teamName}`".

"A caller is waiting to be connected." is just the placeholder shown while the AI call summary is still generating — once it's ready, the card swaps in that summary instead, so you usually walk in already knowing who's calling and why.

- **Accept** — the button shows "Connecting" while the call is set up, then opens the call in a new tab as the [Live Call Detail](/voice-agents/live-calls-board) page — the same live-call screen used by the Live Calls board. If your browser blocks the popup, the call falls back to the in-page softphone corner widget instead.
- **Decline** — you pass; other available agents can still pick it up.
- Nobody accepts before the countdown (about 30 seconds) plus a short grace period ends — the call falls back to the **Fallback Phone Number**, if configured, and the caller hears a single apology from the AI before the handoff.

If you have the same account open in multiple tabs, accepting, declining, or letting the card expire in one tab dismisses it in all the others — you won't get rung in a second tab for a call you already handled. And if a call goes unanswered, the retry rings a *different* available teammate rather than showing the same person an identical second card — so on a single-agent team, a missed card goes straight to the fallback number instead of ringing you again.

While a call is ringing and your tab isn't focused, Revve gets your attention with an OS notification (click it to jump to the tab), a flashing tab title, a ringtone, and a favicon that swaps to a green phone icon. The first time you turn on **Accept Calls**, your browser will ask for notification permission — allow it so you don't miss calls in a background tab.

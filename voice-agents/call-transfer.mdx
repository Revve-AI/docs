...
The first time you turn it on, your browser asks for notification permission — accept it. With permission granted, an incoming transfer card that arrives while you're on another tab triggers a desktop notification, a flashing tab title, and a ringtone, so you don't miss it.
...
The caller hears hold music the whole time the card is open — the hold time follows the card's own countdown, not a fixed wait. If nobody accepts, the flow tries once more with a fresh card, skipping the teammate who just missed or declined it, before falling back.

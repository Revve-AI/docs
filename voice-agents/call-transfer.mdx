...
While the caller waits to be connected, they hear hold music instead of dead air — this applies to every **Transfer To** option, including **Revve User**, not just SIP or phone transfers. For **SIP Address** and **Phone Number** transfers using the default **Bridge Transfer** type, there's an extra step between 3 and 4: the agent privately briefs the human before connecting the caller — see [Bridge or cold transfer](#bridge-or-cold-transfer) below.
...
<Tip>
The first time you turn **Accept Calls** on, your browser will prompt you to allow notifications. Accept it — that's what lets Revve alert you about an incoming transfer if you're working in another tab (see Step 3).
</Tip>
...
If your tab is in the background when the card appears, Revve makes sure you don't miss it: it fires a browser notification (click it to jump to the tab), flashes the tab title to "📞 Incoming call…", and plays a ringtone. All three stop automatically as soon as the request is accepted, declined, or expires.
...
If you're the only teammate with **Accept Calls** on and you let the countdown expire, the transfer doesn't ring you again with a second card — it goes straight to the fallback number (or fails, if none is configured). Revve excludes teammates who already timed out on a given transfer, so a solo agent only sees the card once per call.
...
- [Live Calls Monitoring](/voice-agents/live-calls-monitoring) — listen in on or take over any live call, transferred or not — including the one you just handed off.

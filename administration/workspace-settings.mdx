...
Settings pages are organized as cards, and **each card has its own Save button** that enables only when you've changed something in that card. Saving one card does not save the others — if you edit Working Hours and the Call Window in one sitting, save both.
...
| **Delay (seconds)** (Auto-save Delay card) | 2 seconds (the field's helper text says 1.5) | How long after you stop typing before agent settings auto-save. Range 1–5 in 0.1 steps. Raise it if edits are saving mid-thought; lower it if you keep navigating away before changes persist. |

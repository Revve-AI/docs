...
|--------|---------------|
| **Agent** | The team member and their email. |
| **Groups** | Group membership chips — custom rules can target a specific group. |
| **Status** | An enable/disable switch. Only **enabled** agents receive routed conversations and tickets. |
...
The **Rules** tab starts with four preset rule cards. Each card has its own **Save** button that enables when you change something. These presets cover most teams — configure them before you write any custom rules.
...
- **Rule name** (required — e.g. "VIP Customers", "Technical Support") and an optional description.
...
4. **Queue** — if nobody in the pool is available (everyone disabled, at their limit, or outside working hours), the conversation queues unassigned (when queuing is on) and the customer sees your out-of-office or wait-time message.

"Available" throughout means: in the pool, **Status** enabled, and under the Workload Distribution limit (or the Ticket Distribution limit, for tickets). What the assigned team member sees on their end is covered in [Human Takeover](/inbox/human-takeover).

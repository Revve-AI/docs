...
![The Phone Numbers tab with the Twilio provider card and the number inventory table showing label, source, region, SMS, voice agent, and trend columns](/screenshots/phone-numbers-list.png)
...
| **Enable Voice + inbound voice agent** | Whether the number accepts inbound calls, and which voice agent picks up when someone dials it. This is how you put an agent "on" a number — build the agent first ([Creating Your First Voice Agent](/voice-agents/creating-your-first-voice-agent)), then assign it here. |

...
![The Ask Revve dock with recent conversation tabs at the bottom right of the dashboard](/screenshots/evaluations-scorecards.png)
...
| **Act directly** | Create a voice agent; create [evaluation metrics](/evaluations/evaluation-metrics) and attach them to an agent; create custom contact fields; create a [draft campaign](/campaigns/creating-a-campaign) and attach an agent; save skills | None — happens immediately |
...
Direct actions create *new* things — a new agent, a new metric, a new draft campaign — so there's nothing live to break. Anything that would modify an existing agent's behavior goes through the proposal flow below.
...
- Applying a proposal requires the **Manage Agents** permission. Without it, you'll see "You need the 'Manage Agents' permission to apply this change." — you can still discuss and refine proposals, but a permitted teammate has to apply them.
...
| Direct actions (create agents, metrics, draft campaigns) | Any team member |
